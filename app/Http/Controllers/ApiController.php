...
//            )->where('created_by', $user->id)->get()->toArray();
    ...
    public function mpesaCallback(Request $request)
    {
        ...
        if (!$callbackData || !isset($callbackData->Body->stkCallback)) {
            \Log::error('Invalid M-Pesa callback data received');
        ...
        if ($resultCode != 0) {
            \Log::error("M-Pesa transaction failed: $resultDesc");
        ...
        if ($updated) {
            \Log::info("Transaction updated successfully: CheckoutRequestID = $checkoutRequestID, M-Pesa Code = $mpesaReceiptNumber");
        } else {
            \Log::error("Failed to update transaction: CheckoutRequestID = $checkoutRequestID");
        }
    }
}
