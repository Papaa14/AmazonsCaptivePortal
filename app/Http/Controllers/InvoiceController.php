...
class InvoiceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $invoices = Invoice::where('created_by', \Auth::user()->creatorId())->paginate(10);
    ...
    public function create($customerId = 0)
    {
        ...
        // Define category options (Package or Installation)
        $categories = [
            'package' => 'Package Fee',
            'installation' => 'Installation Fee',
        ];

        return view('invoice.create', compact('customers', 'invoice_number', 'customFields', 'customerId', 'categories'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = \Validator::make(
            ...
            [
                ...
                'category' => 'required|in:package,installation', // Only allow package or installation
                'package_id' => 'required_if:category,package|exists:packages,id', // Validate package selection
            ]
        );
        ...
        $invoice->status = 'Unpaid'; // Default to unpaid
        $invoice->issue_date = $request->issue_date;
        $invoice->due_date = $request->due_date;
        $invoice->category = $request->category; // Either 'package' or 'installation'
        ...
        if ($request->category == 'package') {
            InvoicePackage::create([
                'invoice_id' => $invoice->id,
                'package_id' => $request->package_id,
                'price' => Package::find($request->package_id)->price, // Fetch package price
            ]);
        }
        ...
        if (!empty($setting['invoice_notification']) && $setting['invoice_notification'] == 1) {
            Utility::send_slack_msg('new_invoice', $invoiceNotificationArr);
        }
        if (!empty($setting['telegram_invoice_notification']) && $setting['telegram_invoice_notification'] == 1) {
            Utility::send_telegram_msg('new_invoice', $invoiceNotificationArr);
        }
        if (!empty($setting['twilio_invoice_notification']) && $setting['twilio_invoice_notification'] == 1) {
    ...
    public function show($ids)
    {
        ...
        try {
            ...
            ToastMagic::error('Invoice not found.');
            return redirect()->back();
        }


        $invoice = Invoice::with(['customer', 'payments', 'packages.package'])->find($id);

        if (!$invoice || $invoice->created_by !== \Auth::user()->creatorId()) {
            ToastMagic::error('Invoice not found or access denied.');
            return redirect()->back();
        }
        ...
        $company = $invoice->creator;

        return view('invoice.view', compact('invoice', 'customer', 'invoicePayment', 'packages', 'company'));
    }
