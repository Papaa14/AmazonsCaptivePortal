    ...
    public function userRequest($plan_id)
    {
        ...
        if($objUser->requested_plan == 0)
        {
            ...
            if(!empty($planID))
            {
                ...
                $data->update();
                ToastMagic::success('Request Sent Successfully.');
                return redirect()->back();
            }
            else
            {
        ...
        {
            ToastMagic::error('You already sent a request to another plan.');
            return redirect()->back();
        }
    }
...
//                    $assignPlan = $user->assignPlan($plan_request->plan_id, $plan_request->duration);
                    ...
                    if($assignPlan['is_success'] == true && !empty($plan))
                    {
                        ...
                        ToastMagic::success('Plan successfully upgraded.');
                        return redirect()->back();
                    }
                    else
                    {
                        ToastMagic::error('Plan fail to upgrade.');
                        return redirect()->back();
                    }
                }
                else
                {
                    ...
                    $plan_request->delete();
                    ToastMagic::success('Request Rejected Successfully.');
                    return redirect()->back();
                }
            }
            else
    ...
    */
    public function cancelRequest($id)
    {
        ...
        ToastMagic::success('Request Canceled Successfully.');
        return redirect()->back();
    }
}
