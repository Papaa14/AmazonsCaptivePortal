...
class SystemPaymentController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($objUser->type == 'super admin') {
            ...
            ])->join('users', 'orders.user_id', '=', 'users.id')->orderBy('orders.created_at', 'DESC')->with('total_coupon_used.coupon_detail')->with(['total_coupon_used.coupon_detail'])->paginate(10);
            ...
            $orders = Order::select([
                'orders.*',
                'users.name as user_name',
            ])->join('users', 'orders.user_id', '=', 'users.id')->orderBy('orders.created_at', 'DESC')->where('users.id', '=', $objUser->id)->with('total_coupon_used.coupon_detail')->with(['total_coupon_used.coupon_detail'])->paginate(15);
    ...
    public function refund(Request $request, $id, $user_id)
    {
        ...
        ToastMagic::success('We successfully planned a refund and assigned a free plan.');
        ...
        if (isset($admin_payment_setting) && !empty($admin_payment_setting)) {
            if (
                $admin_payment_setting['is_manually_payment_enabled'] == 'on' ||
                    ...
                    $admin_payment_setting['is_mpesa_enabled'] == 'on' ||
                ...
                if(\Auth::user()->type == 'company'){
                    ...
                    if ($plan) {
                        ...
                        ToastMagic::error(__('Plan is deleted.'));
                        return redirect()->back();
                    }
                } else {
                    ToastMagic::error(__('Permission denied.'));
                    return redirect()->back();
                }
            } else {
                ToastMagic::error(__('The admin has not set the payment method.'));
    ...
    public function stripePost(Request $request)
    {
        ...
        if ($plan) {
                ...
                if (!empty($request->coupon)) {
                    ...
                    if (!empty($coupons)) {
                        ...
                        if ($coupons->limit == $usedCoupun) {
                            ToastMagic::error(__('This coupon code has expired.'));
                            return redirect()->back();
                        }
                    } else {
                        ToastMagic::error(__('This coupon code is invalid or has expired.'));
                        return redirect()->back();
                    }
                }
                ...
                if ($data['amount_refunded'] == 0 && empty($data['failure_code']) && $data['paid'] == 1 && $data['captured'] == 1) {
                    ...
                    if ($data['status'] == 'succeeded') {
                        ...
                        if ($assignPlan['is_success']) {
                            ...
                            ToastMagic::error('Your payment has failed.');
                            return redirect()->route('plans.index');
                        }
                    } else {
                        ToastMagic::error('Your payment has failed.');
                    ...
                    ToastMagic::error('Transaction has been failed.');
                    return redirect()->route('plans.index');
                }
            } catch (\Exception $e) {
            ...
            ToastMagic::error('Plan is deleted.');
            return redirect()->route('plans.index');
        }
    ...
    public function addPayment(Request $request, $id)
    {
        ...
        if ($invoice) {
            if ($request->amount > $invoice->getDue()) {
                ToastMagic::error('Invalid amount.');
                return redirect()->back();
            } else {
                try {
                    ...
                    if ($data['amount_refunded'] == 0 && empty($data['failure_code']) && $data['paid'] == 1 && $data['captured'] == 1) {
                        ...
                        if ($webhook) {
                            ...
                            if ($status == true) {
                                ToastMagic::success('Payment successfully added!');
                                return redirect()->back();
                            } else {
                                ToastMagic::error('Webhook call failed.');
                                return redirect()->back();
                            }
                        }
                        ToastMagic::success('Payment successfully added.');
