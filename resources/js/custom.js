...
$(document).on('click', 'a[data-offcanvas="true"], button[data-offcanvas="true"], div[data-offcanvas="true"]', function () {
    console.log("Offcanvas trigger clicked"); // Debugging
    var data = {};
    var title1 = $(this).data("title");
    var title2 = $(this).data("bs-original-title");
    var title3 = $(this).data("original-title");
    ...
    $("#offcanvasEndLabel").html(title); // Set offcanvas title

    // AJAX request to load content into offcanvas
    $.ajax({
        url: url,
        data: data,
        success: function (data) {
            $('#offcanvasEnd .offcanvas-body').html(data);
            $('#offcanvasEnd').offcanvas('show');
            taskCheckbox();
            common_bind("#offcanvasEnd");
            validation();
            commonLoader();
